...
ADD https://nodejs.org/dist/v6.10.0/node-v6.10.0.tar.gz /usr/local/
...
RUN tar -xf /usr/local/node-v6.10.0.tar.gz && rm /usr/local/node-v6.10.0.tar.gz

ENV MVN_HOME /usr/local/apache-maven-3.3.9
ENV JAVA_HOME /usr/local/jdk1.7.0_79
ENV PATH /usr/local/node-v6.10.0/bin:${JAVA_HOME}/bin:${MVN_HOME}/bin:$PATH
